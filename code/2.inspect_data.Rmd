...
t.alive.oryx <- alive.oryx %>% dplyr::select(collar_ID, date) %>% 
  nest_by(collar_ID) %>% 
  mutate(min_date = map(data, min),
         max_date = map(data, max)) %>% unnest(min_date, max_date) %>%                                                                         mutate(range_time = difftime(max_date, min_date)) %>% 
  dplyr::select(collar_ID, min_date, max_date, range_time)
...
long.alive.oryx <- long.alive.oryx[,1:18]
...
long.alive.oryx$date_collared <- long.alive.oryx$date_collared %>% mdy()
long.alive.oryx <- long.alive.oryx %>% dplyr::filter(!is.na(date))
long.alive.oryx <- long.alive.oryx %>% drop_na() # drop the nas

long.alive.oryx %>% summary()

# save the data
#write_rds(long.alive.oryx, path = here("data","long.healthy.Rdata"))
```
...
oryx.disease$date_collared <- oryx.disease$date_collared %>% mdy()
oryx.disease <- oryx.disease %>% dplyr::filter(!is.na(date))
oryx.disease <- oryx.disease %>% drop_na() # drop the nas

oryx.disease %>% summary()

# save the data
#write_rds(oryx.disease, path = here("data","disease.Rdata"))
```
...
```{r amt conversion}
# make tracks 
trk.alive <- amt::make_track(long.alive.oryx, .x=long, .y=lat, .t=date, masterID = masterID, collar_id= collar_ID, sex= sex, fate = fate, date_collared = date_collared, date_released = date_released, temp_c = temp_c, prev_collar_ID = prev_collar_ID, date_released = date_released, cause_of_death = cause_of_death) # healthy 

trk.disease <- amt::make_track(oryx.disease, .x=long, .y=lat, .t=date, masterID = masterID, collar_id= collar_ID, sex= sex, fate = fate, date_collared = date_collared, date_released = date_released, temp_c = temp_c, prev_collar_ID = prev_collar_ID, date_released = date_released, cause_of_death = cause_of_death) # disease
...
trk1.alive <- trk.alive %>% nest_by(collar_id) %>% 
...
# nest data by id and add columns to each nested column of data (sl and nsd)
trk1.disease <- trk.disease %>% nest_by(collar_id) %>% 
  mutate(sl_ = list(step_lengths(data)),
...
disease.kde.week <- trk.disease %>% mutate(week = week(t_)) %>% nest_by(collar_id, week) %>% 
  transmute(kdearea = list(hr_kde(data, levels = 0.95) %>% hr_area)) %>% 
  unnest()

alive.kde.week <- trk.alive %>% mutate(week = week(t_)) %>% nest_by(collar_id, week) %>% 
  transmute(kdearea = list(hr_kde(data, levels = 0.95) %>% hr_area)) %>% 
  unnest()

...
```{r amt conversion}
# make tracks 
trk.disease <- amt::make_track(oryx.disease, .x=long, .y=lat, .t=date, id = collar_ID, sex = sex, fate = fate, est_DOD = est_DOD) # disease
trk.alive <- amt::make_track(long.alive.oryx, .x=long, .y=lat, .t=date, id= collar_ID, sex= sex, fate = fate) # healthy 

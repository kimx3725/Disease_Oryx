...
hmm <- readr::read_rds(here::here("data/HMM_outputs", "hmm_sim2_output.rds"))
sim_sequence <- readr::read_rds(here::here("data/hmm_sequence_data", "HMM_sim2_sequence.rds"))

hmm
colnames(sim_sequence)[8] <- "null_state"
colnames(sim_sequence)[10] <- "est_state"
sim_sequence
```
...
```{r plot1}
fig9 <- ggplot(hmm_estimate_tpm, aes(x = factor(param), y = est, ymin = est-lower, ymax = est+upper, color = factor(param)))+
  geom_pointrange()+
...
fig10a <- ggplot(data = melted_trProb, aes(y = reorder(From, desc(From)), x= To, fill = est_value)) +
...
fig10b <- ggplot(data = melted_trProb, aes(y = reorder(From, desc(From)), x= To, fill = true_value)) +
  geom_tile(color = "black") +
  scale_fill_gradient2(
    low = "white", high = "#b30000",
    limit = c(0, 1), space = "Lab",

...
*Logistic Regression Models*: You now define the betas for each state transition according to the provided values.

*Transition Probability Computation*: The compute_transition_probabilities function uses these betas to compute transition probabilities based on the shrub cover.

*Simulation Loop Update*: The loop now uses the computed transition probabilities to determine the next state for each day. This updated code ensures that the transition probabilities are modeled on the logit scale as a function of shrub cover, with the specified betas defining these relationships.
...
y4 <- dgamma(x = x, shape = 0.14, 660) # IR
y5 <- dgamma(x = x, shape = 0.05, 10) # D
...
# Step 1: Define Logistic Regression Models for Transition Probabilities

First, define the logistic regression models for each transition probability as functions of shrub cover.
```{r transitional matrix}
logit <- function(p) log(p / (1 - p))

inv_logit <- function(x) exp(x) / (1 + exp(x))

# Define betas for logistic regression models
betas <- list(
  beta_12 = c(-1, 1.2),   # From state 1 to state 2
  beta_21 = c(-0.5, -0.5),  # From state 2 to state 1
  beta_24 = c(-1.5, 2),   # From state 2 to state 4
  beta_25 = c(-4, 3),   # From state 2 to state 5
  beta_34 = c(-1.5, 2.5),   # From state 3 to state 4
  beta_43 = c(-0.5, 1.5),  # From state 4 to state 3
  beta_45 = c(-4, 3)    # From state 4 to state 5
)
```

# Step 2: Define a Function to Compute Transition Probabilities
Update the function to compute transition probabilities.

```{r transitional probabilities}
# Define the function to compute transition probabilities
compute_transition_probabilities <- function(shrub_cover, betas) {
  # Ensure shrub_cover is numeric
  shrub_cover <- as.numeric(shrub_cover)
  
  # Compute logit values
  logits <- sapply(betas, function(beta) beta[1] + beta[2] * shrub_cover)
  
  # Convert logits to probabilities using inverse logit function
  probs <- inv_logit(logits)
  
  # Create a transition probability matrix
  trans_m <- matrix(0, nrow = 5, ncol = 5)
  trans_m[1, 2] <- probs["beta_12"]
  trans_m[1, 1] <- 1 - trans_m[1, 2]
  
  trans_m[2, 1] <- probs["beta_21"]
  trans_m[2, 4] <- probs["beta_24"]
  trans_m[2, 5] <- probs["beta_25"]
  trans_m[2, 2] <- max(1 - trans_m[2, 1] - trans_m[2, 4] - trans_m[2, 5], 0)
  
  trans_m[3, 4] <- probs["beta_34"]
  trans_m[3, 3] <- 1 - trans_m[3, 4]
  
  trans_m[4, 3] <- probs["beta_43"]
  trans_m[4, 5] <- probs["beta_45"]
  trans_m[4, 4] <- max(1 - trans_m[4, 3] - trans_m[4, 5], 0)
  
  trans_m[5, 5] <- 1 # State 5 (death) is absorbing
  
  # Ensure no negative probabilities
  trans_m <- pmax(trans_m, 0)
  
  # Normalize probabilities to sum to 1
  trans_m <- t(apply(trans_m, 1, function(row) row / sum(row)))
...
## model 4 - infected & resting
  m_ir <- make_issf_model(coefs = c("final.shrub_end" = 0, 
                                    ...
                                    "log(sl_):final.shrub_start" = 0, 
                                    "cos(ta_):final.shrub_start" = 0),
                          sl = make_gamma_distr(shape = 0.14,
                                                scale = 660),
                          ta = make_vonmises_distr(kappa = 0.0001))
...
## model 5 - death 
  m_d <- make_issf_model(coefs = c("final.shrub_end" = 0, 
                                    ...
                                    "log(sl_):final.shrub_start" = 0, 
                                    "cos(ta_):final.shrub_start" = 0),
                          sl = make_gamma_distr(shape = 0.05,
                                                scale = 10),
...
# Step 3: Modify the Simulation Loop to Use the Logistic Regression Models
Update your simulation loop to use the compute_transition_probabilities function.
...
for (ind in 1:n_ind) {
  ...
  for (i in 1:total_days) {
    ...
    if (i == 1) {
      ...
      # Compute the transition probability matrix based on current shrub cover
      trans_m <- compute_transition_probabilities(shrub_cover, betas)
...
# save the data
saveRDS(ssf_df, here::here("data/simulation_data", "sim3_trk_10ids_3.rds"))
```

...
# since our coordinate systems are in utm -- we will convert it to the lat long 
x1_coords <- ssfdat %>% st_as_sf(., coords = c("x1_", "y1_"), crs = "epsg:32634") %>% st_transform(., crs = "epsg:4326") %>% st_coordinates()
colnames(x1_coords) <- c("coords.x1", "coords.y1")

x2_coords <- ssfdat %>% st_as_sf(., coords = c("x2_", "y2_"), crs = "epsg:32634") %>% st_transform(., crs = "epsg:4326") %>% st_coordinates()
colnames(x2_coords) <- c("coords.x2", "coords.y2")

ssfdat1 <- cbind(ssfdat, x1_coords, x2_coords)
ssfdat1

# save the data 
#write_rds(ssfdat1, path = here("data/finalized_data", "multiple_trk_modified.Rdata"))

# select applicable columns and rename them 

## use locations 
use <- ssfdat1 %>% dplyr::filter(case_ == TRUE) %>% dplyr::select(coords.x1, coords.y1, t1_, case_, masterID) %>% rename_with(.cols = c(1:5), ~c("x_","y_","t_","case", "id"))
use

## random locations
random <- ssfdat1 %>% dplyr::filter(case_ == FALSE) %>% dplyr::select(coords.x2, coords.y2, t2_, case_, masterID) %>% rename_with(.cols = c(1:5), ~c("x_","y_","t_","case", "id"))
random
```

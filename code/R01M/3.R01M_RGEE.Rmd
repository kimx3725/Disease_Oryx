...
R01M <- read_rds(here::here("data/finalized_data", "trk.ssf_R01M.Rdata"))

# select applicable columns and rename them 

# use locations 
use <- R01M %>% dplyr::filter(case_ == TRUE) %>% dplyr::select(x1_, y1_, t1_, case_) %>% rename_with(.cols = c(1:4), ~c("x_","y_","t_","case"))

# random locations
random <- R01M %>% dplyr::filter(case_ == FALSE) %>% dplyr::select(x2_, y2_, t2_, case_) %>% rename_with(.cols = c(1:4), ~c("x_","y_","t_","case"))
```
...
```{r used extract NDVI}
use$uniq <- rep(1:1000, each=1000)[1:nrow(use)] #This is for up to 1 million points. To increase the max number of points, increase the value for max repetitions. To change the number of points to run per time, change the value in the argument each (up to 5000).
...
# save the data 
#write_rds(use.NDVI.df, path = here("data/env_data", "SSF_NDVI_use.Rdata"))
```

## Random locations NDVI
```{r random extract NDVI}
random$uniq <- rep(1:1000, each=1000)[1:nrow(random)] #This is for up to 1 million points. To increase the max number of points, increase the value for max repetitions. To change the number of points to run per time, change the value in the argument each (up to 5000).
...
# save the data 
#write_rds(random.NDVI.df, path = here("data/env_data", "SSF_NDVI_random.Rdata"))
```
...
# save the data 
#write_rds(use.PRC.df, path = here("data/env_data", "SSF_PRC_use.Rdata"))
```
...
# save the data 
#write_rds(random.PRC.df, path = here("data/env_data", "SSF_PRC_random.Rdata"))
```

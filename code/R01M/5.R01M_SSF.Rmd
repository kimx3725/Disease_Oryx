...
m1 <- amt::fit_issf(case_ ~NDVI+ total_precipitation+ NDVI:avg.bc+ total_precipitation:avg.bc+ NDVI:speed+ total_precipitation:speed+ log(sl_)+ cos(ta_)+ avg.bc:log(sl_)+ avg.bc:cos(ta_)+ strata(step_id_), data = trk.R01M)
...
- **NDVI:avg.bc**: *positive* as average body condition increases, the animal would likely to be in a higher NDVI places.

...
sim_df <- readr::read_rds(here::here("data/simulation_data", "sim2_df.rds"))
...
shrub <- terra::extract(shrub, sim_df[, c("x", "y")]) 
sim_df$shrub <- shrub[, -1]
...
data_hmm <- momentuHMM::prepData(sim_df, type = "UTM", covNames = c("state", "shrub"))
...
Par0_5s <- list(       # mean
                step = c(2300, 245, 452, 245, 10,
                       # sd
                         2300, 245, 452, 245, 10), 
                angle = c(0.04, 0.02, 0.1, 0.03, 0.02)
                )
...
# save the hmm output 
saveRDS(hmm, here::here("data/HMM_outputs", "hmm_sim2_output.rds"))
```
...
saveRDS(data_hmm, here::here("data/hmm_sequence_data", "HMM_sim2_sequence.rds"))
```

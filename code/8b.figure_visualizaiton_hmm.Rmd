...
dose.labs <- c("i_11" = "Infected 11", 
               "i_22" =  "Infected 22", 
               "u_20" =  "Uknown 20", 
               "u_37" =  "Uknown 37")

# new facet label names for supp variable 
supp.labs <- c("pool" = "Complete pool", 
               "non_pool" = "No pool")
...
fig3a <- ggplot(fig, mapping = aes(x, y, col = state, group = new_ID))+
  ...
  facet_grid(new_ID~group,
             labeller = labeller(new_ID = dose.labs, group = supp.labs))+
...
```{r combine the plots}
# combine the figures 
grid <- cowplot::plot_grid(fig3a, fig3b, nrow = 1, rel_widths = c(1,2), align = "v")
...
fig1 <- fig %>% mutate(julian = yday(time),

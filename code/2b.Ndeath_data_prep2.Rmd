...
Reformat the data and filter the necessary data inputs only -- The locations need to be projected from long-lat to E-N prior to analysis, as the package uses Euclidean (planar) eometry to derive metrics such as step lengths and turning analges. They also need to be at a regular time interval. 
```{r data reformat}
...
include death status information a total 7 individuals: 
R02M	1			
R38F	4
R42M	18
B49M	24
...
#readr::write_rds(whole_df, path = here("data/HMM", "HMM_prep_data_Ndeath.Rdata"))
```

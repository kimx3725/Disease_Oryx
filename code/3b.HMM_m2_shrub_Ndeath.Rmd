...
oryx <- readr::read_rds(here::here("data/HMM", "HMM_prep_data_Ndeath.Rdata"))
oryx

summary(oryx)
  
# Location data of the oryx 
oryx_id <- readr::read_rds(here::here("data/filtered_dataset", "oryx_trk.Rdata"))
oryx_id
#B15F, R02M, R08M, R12F, R38F
death_index <- oryx_id %>% distinct(masterID, num_id, cause_of_death)
death_index <- death_index %>% filter(num_id %in% oryx$ID)

#saveRDS(death_index, "cause_of_death_index.Rdata")
```
...
hist(data_hmm$step, xlab = "step length", main = "")
...
ggplot(data_hmm, aes(x = angle))+
  facet_wrap(~status)+
  geom_histogram(col = "white", fill = "grey", bins = 20)+
  scale_x_continuous(breaks=seq(-pi, pi, 15))

hist(data_hmm$angle, breaks = seq(-pi, pi, length = 15), xlab = "angle", main = "")
```

get the initial movement parameters 
```{r movement params}
data_hmm %>% filter(step < 5) %>% 
  dplyr::summarize(step_mean = mean(step, na.rm = TRUE),
                   ta_mean = mean(angle, na.rm = TRUE))

# non infected 
data_hmm %>% filter(status == "n" & step < 500) %>%   
  dplyr::summarize(step_mean = mean(step, na.rm = TRUE),
                   ta_mean = mean(angle, na.rm = TRUE))

# infected 
data_hmm %>% filter(status == "i" & step < 100) %>%   
  dplyr::summarize(step_mean = mean(step, na.rm = TRUE),
                   ta_mean = mean(angle, na.rm = TRUE))
# death
data_hmm %>% filter(death == "5") %>%   
  dplyr::summarize(step_mean = mean(step, na.rm = TRUE),
                   ta_mean = mean(angle, na.rm = TRUE))
```
...
first fit the simple model without any covariate to get the initial parameters.
```{r 5 state hmm no covariates}
# number of states 
nbStates <- 5
...
# save the hmm output 
saveRDS(hmm, "hmm_m2_output.rds")
#hmm <- readr::read_rds(here::here("data/hmm_real_outputs", "pool_hmm_shrub.rds"))
```
...
saveRDS(data_hmm, "HMM_m2_sequence.rds")
```

...
1. Determine an appropriate disease predictor from the data. 
2. Determine the best way to merge the disease predictor and GPS tracking locations into a single data frame for further model fits. 
...
```{r remove columns}
# filter data 
oryx.gps.df <- oryx.gps %>% dplyr::select("collar_ID", "utm_e", "utm_n", "date", "long", "lat", "DOP")
...
oryx.id.df <- oryx.id.df %>% dplyr::select(masterID, Rgroup, collar_ID, date_collared, sex, DOB, date_arrived, date_released, age_at_release, birth_type, dam_masterID, dam_Rgroup, act_dropoff, fate, mort_detected, est_DOD, collar_malfunction, age_at_death, cause_of_death)

# convert NAs to 0 from the cause of death column
...
oryx.filter.id <-  oryx.id.df %>% 
          ...
          unnest(birth_type ,DOB, sex, fate, date_released, age_at_release,death, age_death, est_DOD) %>% dplyr::select(-data)
